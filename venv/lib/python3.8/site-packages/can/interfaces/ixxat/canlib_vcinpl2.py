...
try:
    ...
    _canlib.map_symbol(
        "canChannelWaitTxEvent", hresult_type, (HANDLE, ctypes.c_uint32), __check_status
    )
    # HRESULT canChannelWaitRxEvent (HANDLE hChannel, UINT32 dwMsTimeout );
    _canlib.map_symbol(
        "canChannelWaitRxEvent", hresult_type, (HANDLE, ctypes.c_uint32), __check_status
    )
...
class IXXATBus(BusABC):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        log.info(
            "Initializing control %d with SDR={%s}, FDR={%s}", channel, pBtpSDR, pBtpFDR
        )
    ...
    def _canptb_build(defaults, bitrate, tseg1, tseg2, sjw, ssp):
        ...
        return structures.CANBTP(
            dwMode=dw_mode, dwBPS=bitrate, wTS1=tseg1, wTS2=tseg2, wSJW=sjw, wTDO=ssp
        )
