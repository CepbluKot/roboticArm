...
CSI_DestroyProtocolTree.argtypes = [ctypes.POINTER(CSI_Tree)]
...
OCI_GetError.argtypes = [OCI_Handle, OCI_ErrorCode, ctypes.c_char_p, ctypes.c_uint32]
...
OCI_GetTimerValue.argtypes = [OCI_ControllerHandle, ctypes.POINTER(OCI_Time)]
...
class OCI_CANFDRxConfig(ctypes.Structure):
    _fields_ = [("canRxMode", OCI_CANRxMode), ("canFdRxMode", OCI_CANFDRxMode)]
...
class OCI_CANControllerProperties(ctypes.Structure):
    _fields_ = [("mode", OCI_ControllerPropertiesMode)]
...
class OCI_CANControllerStatus(ctypes.Structure):
    _fields_ = [("reserved", ctypes.c_uint32), ("stateCode", ctypes.c_uint32)]
...
class OCI_CANRxCallbackSingleMsg(ctypes.Structure):
    ...
    _anonymous_ = ("u",)
    _fields_ = [("u", _U), ("userData", ctypes.c_void_p)]
