    ...
    if isinstance(abc_or_property, type):
        ...
        if abc_or_property in {abc.model.Dictionary, abc.model.Array, abc.model.Object}:
            ...
            class_or_property = getattr(
                serial.model, abc_or_property.__name__.split(".")[-1]
            )
        ...
        for types_attribute in ("types", "value_types", "item_types"):
            ...
            if hasattr(class_or_property, types_attribute):
                ...
                setattr(
                    ...
                    tuple(
                        ab2c(type_)
                        for type_ in getattr(class_or_property, types_attribute)
                    ),
                )
...
def marshal(
    ...
    if hasattr(data, "_marshal"):
        return (
            data._marshal()
        )  # noqa - this is *our* protected member, so linters can buzz off
...
class _Unmarshal(object):
    ...
    def __init__(
        ...
        if not isinstance(data, UNMARSHALLABLE_TYPES):
            ...
            raise errors.UnmarshalTypeError(
                ...
                + ", ".join(
                    (qualified_name(data_type) for data_type in UNMARSHALLABLE_TYPES)
                )
            )
        ...
        self.value_types = (
            value_types
        )  # type: Optional[Sequence[Union[type, properties.Property]]]
        self.item_types = (
            item_types
        )  # type: Optional[Sequence[Union[type, properties.Property]]]
    ...
    def __call__(self):
        ...
        if (self.data is not None) and (self.data is not properties.NULL):
            ...
            if (
                self.meta is None
            ):  # Only un-marshall models if they have no metadata yet (are generic)
                ...
                if self.types is None:
                    ...
                    for type_ in self.types:
                        ...
                        error = (
                            None
                        )  # type: Optional[Union[AttributeError, KeyError, TypeError, ValueError]]
    ...
    def backport_types(self):
        ...
        if (
            ...
            and (native_str not in self.types)
        ):
    ...
    def as_type(
        self, type_,  # type: Union[type, properties.Property]
    ):
        # type: (...) -> bool

        unmarshalled_data = (
            None
        )  # type: Union[abc.model.Model, Number, str, bytes, date, datetime]
        ...
        if isinstance(type_, properties.Property):
            ...
            if isinstance(self.data, (dict, collections.OrderedDict, abc.model.Model)):

                if issubclass(type_, abc.model.Object):

                    unmarshalled_data = type_(self.data)

                elif issubclass(type_, abc.model.Dictionary):

                    unmarshalled_data = type_(self.data, value_types=self.value_types)

                elif issubclass(type_, (dict, collections.OrderedDict)):
                ...
                self.data,
                (collections_abc.Set, collections_abc.Sequence, abc.model.Array),
            ) and (not isinstance(self.data, (str, bytes, native_str))):
                ...
                if issubclass(type_, abc.model.Array):
                    ...
                    type_, (collections_abc.Set, collections_abc.Sequence)
                ) and not issubclass(type_, (str, bytes, native_str)):
...
def deserialize(data, format_):
    ...
    if format_ not in ("json", "yaml"):  # , 'xml'
        raise NotImplementedError(
            "Deserialization of data in the format %s is not currently supported."
            % repr(format_)
        )
...
def validate(
                    ...
                    "\n".join(
                        chain(
                            ...
                            (
                                "    %s,"
                                % "\n".join(
                                    "    " + line for line in repr(type_).split("\n")
                                ).strip()
                                for type_ in types
                            ),
                            ("  )",),
    ...
    if ("_validate" in dir(data)) and callable(data._validate):
        ...
        error_messages.extend(
            error_message
            for error_message in data._validate(raise_errors=False)
