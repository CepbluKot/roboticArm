...
def model(
    ...
    if isinstance(model_instance, serial.model.Object):
        ...
        errors = serial.marshal.validate(
        ...
        try:
            ...
            for k, a_b in _object_discrepancies(
                model_instance, reloaded_model_instance
            ).items():
                ...
                if sa != sb:
                    ...
                    message.append(
                        "\n    %s().%s:\n\n        %s\n        %s\n        %s"
                        % (qualified_model_name, k, sa, "==" if sa == sb else "!=", sb)
                    )
        ...
        for property_name, property in serial.meta.read(
            ...
            if isinstance(property_value, Model) or (
                hasattr(property_value, "__iter__")
                and (not isinstance(property_value, (str, native_str, bytes)))
            ):
                model(
                    property_value,
                    format_=format_,
        ...
        for item in model_instance:
            ...
            if isinstance(item, Model) or (
                ...
                and (not isinstance(item, (str, native_str, bytes)))
            ):
                model(
                    item,
                    format_=format_,
        ...
        for key, value in model_instance.items():
            ...
            if isinstance(value, Model) or (
                ...
                and (not isinstance(value, (str, native_str, bytes)))
            ):
                model(
                    value,
                    format_=format_,
