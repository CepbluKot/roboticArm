...
def qualified_name(type_):
    ...
    if isinstance(type_, _Module):
        ...
        if type_name in ("builtins", "__builtin__", "__main__", "__init__"):
            type_name = None

    else:

        if type_.__module__ not in ("builtins", "__builtin__", "__main__", "__init__"):
    ...
    if frame_function != "<module>":
        ...
        if arguments:
            ...
            if (
                ...
                and (
                    (argument_value_type.__name__ not in dir(builtins))
                    or (
                ...
                re.sub(
                    ...
                    re.sub(
                        ...
                        re.sub(
                            ...
                            r"\1_\2",
                            re.sub(
                                r"([^\x20-\x7F]|\s)+", "_", normalize("NFKD", string)
                            ),
                        ),
                    ),
                ),
    ...
    for s in string:
        if s in "0123456789":
            ...
            if character_type == 3:
                ...
                words.append([s])
            character_type = 3
    return tuple("".join(w) for w in words)
...
class Undefined(object):
    def __init__(self):
        ...
        if UNDEFINED is not None:
            raise RuntimeError("%s may only be defined once." % repr(self))

    def __repr__(self):
        return (
            "UNDEFINED"
            if self.__module__ in ("__main__", "builtins", "__builtin__", __name__)
            else "%s.UNDEFINED" % self.__module__
        )
...
def read(data):
    ...
    if (hasattr(data, "readall") and callable(data.readall)) or (
        ...
        raise TypeError("%s is not a file-like object" % repr(data))
