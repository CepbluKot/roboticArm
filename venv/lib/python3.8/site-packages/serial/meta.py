...
class Meta(object):
    ...
    def __repr__(self):
        return "\n".join(
            ...
            + ["    %s=%s," % (p, repr(v)) for p, v in properties_values(self)]
            + [")"]
        )
...
class Version(Meta):
    def __init__(
        ...
        if isinstance(version_number, str) and (
            ...
            and (equals is None)
            and (not_equals is None)
            and (less_than is None)
            and (less_than_or_equal_to is None)
            and (greater_than is None)
            and (greater_than_or_equal_to is None)
        ):
            specification = None
    ...
    def __eq__(self, other):
        ...
        if (isinstance(other, str) and _DOT_SYNTAX_RE.match(other)) or isinstance(
            other, (collections_abc.Sequence, int)
        ):
            ...
            if isinstance(other, str):
                ...
                if other == "":
                    ...
                    other_components = tuple(
                        int(other_component) for other_component in other.split(".")
                    )
            else:
...
class Object(Meta):
    def __init__(
        self, properties=None  # type: Optional[Properties]
    ):
...
class Dictionary(Meta):
    def __init__(
        self, value_types=None  # type: Optional[Sequence[Property, type]]
    ):
        ...
        if value_types is not None:
            ...
            if native_str is not str:
                ...
                if isinstance(value_types, collections.Callable):
                    ...
                    (str in value_types)
                    and (native_str is not str)
                    and (native_str not in value_types)
                ):
...
class Array(Meta):
    def __init__(
        self, item_types=None  # type: Optional[Sequence[Property, type]]
    ):
        ...
        if item_types is not None:
            ...
            if native_str is not str:
                if isinstance(item_types, collections.Callable):
                    ...
                    (str in item_types)
                    and (native_str is not str)
                    and (native_str not in item_types)
                ):
                    item_types = chain(
                        *(((t, native_str) if (t is str) else (t,)) for t in item_types)
                    )
...
class Properties(OrderedDict):
    ...
    def __deepcopy__(self, memo=None):
        ...
        return self.__class__(
            tuple((k, deepcopy(v, memo=memo)) for k, v in self.items())
        )
...
def writable(
    ...
    if isinstance(model, Model):
        ...
        if model._meta is None:
            model._meta = (
                Object()
                if issubclass(model, serial.model.Object)
                else Array()
                if issubclass(model, serial.model.Array)
                else Dictionary()
...
def write(
    ...
    metadata_type = (
        Object
        if issubclass(model_type, serial.model.Object)
        else Array
        if issubclass(model_type, serial.model.Array)
        else Dictionary
...
def xpath(model, xpath_=_UNIDENTIFIED):
    ...
    if not isinstance(model, Model):
        ...
        raise TypeError(
            ...
            % (qualified_name(Model), repr(model))
        )
    ...
    if pointer_ is not _UNIDENTIFIED:
        ...
        if isinstance(model, serial.model.Dictionary):
            ...
            for k, v in model.items():
                if isinstance(v, Model):
                    pointer(
                        v, "%s/%s" % (pointer_, k.replace("~", "~0").replace("/", "~1"))
                    )
...
def url(model, url_=_UNIDENTIFIED):
    ...
    if url_ is not _UNIDENTIFIED:
        if not isinstance(url_, str):
            raise TypeError("`url_` must be a `str`, not %s." % repr(url_))
        model._url = url_
        if isinstance(model, serial.model.Dictionary):
    ...
    if serialization_format is not _UNIDENTIFIED:
        ...
        if not isinstance(serialization_format, str):
            ...
            if isinstance(serialization_format, native_str):
                ...
                raise TypeError(
                    "`serialization_format` must be a `str`, not %s."
...
def version(data, specification, version_number):
    ...
    if not isinstance(data, serial.abc.model.Model):
        raise TypeError(
            "The data provided is not an instance of serial.abc.model.Model: "
            + repr(data)
        )
    ...
    def version_property(property):
        ...
        changed = False
        if isinstance(property, serial.properties.Array) and (
            property.item_types is not None
        ):
            ...
            if item_types is not None:
                ...
                changed = True
        elif isinstance(property, serial.properties.Dictionary) and (
            property.value_types is not None
        ):
