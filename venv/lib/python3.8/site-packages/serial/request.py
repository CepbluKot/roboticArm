...
class Headers(object):
    ...
    def __getitem__(self, key):
        ...
        if key == "Content-length":
            ...
            if (
                (value is not None)
                and value.strip().lower()[:9] == "multipart"
                and hasattr(self.request, "boundary")
            ):
    ...
    def copy(self):
        # type: (...) -> Headers
        return self.__class__(self._dict, request=self.request)
    ...
    def data(self, data):
        ...
        if data is not None:
            ...
            if isinstance(data, (Model, dict)) or (
                isinstance(data, (collections_abc.Sequence, collections_abc.Set))
    ...
    def __bytes__(self):
        if self._bytes is None:
            ...
            for k, v in self.headers.items():
                lines.append(bytes("%s: %s" % (k, v), encoding="utf-8"))
            lines.append(b"")
            data = self.data
            if data:
...
class Part(Data):
    ...
    def boundary(self):
        ...
        if self._boundary is None:
            data = b"\r\n".join([self._data or b""] + [bytes(p) for p in self.parts])
            boundary = b"".join(
                bytes(
    ...
    def data(self):
        ...
        if self.parts:
            data = (b"\r\n--" + self.boundary + b"\r\n").join(
                [self._data or b""] + [bytes(p).rstrip() for p in self.parts]
            ) + (b"\r\n--" + self.boundary + b"--")
        else:
            data = self._data
        return data
