...
try:
    ...
    Union = (
        ...
        Any
    ) = AnyStr = IO = Sequence = Mapping = Callable = Tuple = Optional = Set = None
...
class Object(object):
    ...
    def __init__(
        self, _=None  # type: Optional[Union[str, bytes, dict, Sequence, IO]]
    ):
            ...
            if isinstance(_, Object):
                ...
                if isinstance(_, dict):
                    ...
                    if format_ is None:
                        _dir = tuple(
                            property_name
                            for property_name in dir(_)
                            if property_name[0] != "_"
                        )
                        for property_name in meta.writable(
                            self.__class__
                        ).properties.keys():
    ...
    def _validate(self, raise_errors=True):
        ...
        if (instance_hooks is not None) and (
            instance_hooks.before_validate is not None
        ):
            ...
            if value is None:
                ...
                if required:
                    ...
                    validation_errors.append(
                        ...
                        % (property_name, qualified_name(type(object_)), str(object_))
                    )
            else:
                ...
                if value is properties.NULL:
                    ...
                    try:
                        value_validation_error_messages = validate(
                            value, property.types, raise_errors=False
                        )
                        ...
                        if error.args:
                            error.args = tuple(
                                chain((error.args[0] + message,), error.args[1:])
                            )
                        else:
                            error.args = (message,)
...
class Array(list):
    ...
    def __repr__(self):
        ...
        if len(self) > 0:
            ...
            representation.append(
                "    ]" + ("" if m is None or m.item_types is None else ",")
            )
        im = meta.read(self)
        cm = meta.read(type(self))
        if im is not cm:
...
class Dictionary(collections.OrderedDict):
    ...
    def __setitem__(
        ...
        try:
            ...
            unmarshalled_value = unmarshal(value, types=value_types)

        except TypeError as error:

            message = "\n - %s['%s']: " % (qualified_name(type(self)), key)

            if error.args and isinstance(error.args[0], str):

                error.args = tuple(chain((message + error.args[0],), error.args[1:]))
    ...
    def _marshal(self):
        ...
        unmarshalled_data = collections.OrderedDict(
            [(k, marshal(v, types=value_types)) for k, v in data.items()]
        )  # type: collections.OrderedDict
    ...
    def __repr__(self):
        ...
        if len(items) > 0:
            ...
            representation.append(
                "    ]"
                if self._meta is None or self._meta.value_types is None
                else "    ],"
            )
        cm = meta.read(type(self))
        im = meta.read(self)
...
def from_meta(name, metadata, module=None, docstring=None):
    ...
    def typing_from_property(p):
        ...
        if isinstance(p, type):
            if p in (Union, Dict, Any, Sequence, IO):
            ...
            if p.item_types:
                if len(p.item_types) > 1:
                    item_types = "Union[%s]" % (
                        ", ".join(typing_from_property(it) for it in p.item_types)
                    )
                else:
            ...
            if p.value_types:
                if len(p.value_types) > 1:
                    value_types = "Union[%s]" % (
                        ", ".join(typing_from_property(vt) for vt in p.value_types)
                    )
                else:
