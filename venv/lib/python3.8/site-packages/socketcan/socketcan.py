...
class CanFrame:
    ...
    def __init__(self, can_id: int, data: bytes, flags: CanFlags = 0):
        self.can_id = can_id
        self.flags = flags
    ...
    def to_bytes(self):
        ...
        return struct.pack(
            self.FORMAT,
            combine_can_id_and_flags(self.can_id, self.flags),
            len(self.data),
            data,
        )
    ...
    def from_bytes(cls, byte_repr):
        ...
        return CanFrame(can_id=can_id, flags=flags, data=data[:data_length])
...
class CanFdFrame(CanFrame):
    ...
    def from_bytes(cls, byte_repr):
        ...
        """
        assert len(byte_repr) == cls.get_size()
        can_id_with_flags, data_length, fd_flags, data = struct.unpack(
            cls.FORMAT, byte_repr
        )
        can_id, flags = split_can_id_and_flags(can_id_with_flags)
        return CanFdFrame(
            can_id=can_id, flags=flags, fd_flags=fd_flags, data=data[:data_length]
        )
...
class BcmMsg:
    ...
    def to_bytes(self) -> bytes:
        ...
        byte_repr.extend(
            struct.pack(
                ...
                self.count,
                interval1_seconds,
                interval1_microseconds,
                interval2_seconds,
                interval2_microseconds,
                self.can_id,
                len(self.frames),
            )
        )
    ...
    def from_bytes(cls, byte_repr: bytes):
        ...
        (
            ...
            can_id,
            number_of_frames,
        ) = struct.unpack(cls.FORMAT, byte_repr[: cls.get_size()])
...
class CanBcmSocket:
    ...
    def setup_cyclic_transmit(self, frame: CanFrame, interval: float) -> int:
        """
    ...
    def setup_receive_filter(self, frame: CanFrame, timeout: float) -> int:
        """
    ...
    def get_receive_filter(self, can_id: int, can_flags: CanFlags = 0):
        """
...
class IsoTpOpts:
    ...
    def from_bytes(cls, byte_repr: bytes):
        ...
        (
            ...
            rxpad_content,
            rx_ext_address,
        ) = struct.unpack(cls.FORMAT, byte_repr)
        return IsoTpOpts(
            flags=flags,
