...
class ControlServo:
    ...
    def read_speed(self):
        ...
        while decoded_recieve.id != command_answer_id:
            self.servo.send(channel=0, message=read_speed)
    ...
    def set_speed(self, value: int):
        if value != servo_info_storage[self.servo.servo_id].speed:
            ...
            self.servo.send(channel=0, message=set_speed)
    ...
    def read_pos(self):
        ...
        while decoded_recieve.id != command_answer_id:
            self.servo.send(channel=0, message=read_pos)
            recv = self.servo.receive(channel=0)
            decoded_recieve = self.command_reader.read_recieve(recv)
    ...
    def set_pos(self, value: int):
        if servo_info_storage[self.servo.servo_id].pos != value:
            ...
            self.servo.send(channel=0, message=set_pos)
    ...
    def read_mode(self):
        ...
        while decoded_recieve.id != command_answer_id:
            self.servo.send(channel=0, message=read_mode)
            recv = self.servo.receive(channel=0)
            decoded_recieve = self.command_reader.read_recieve(recv)
    ...
    def set_mode(self, value: int):
        if servo_info_storage[self.servo.servo_id].mode != value:
            ...
            # while decoded_recieve.id != command_answer_id:
            self.servo.send(channel=0, message=set_mode)
                # recv = self.servo.receive(channel=0)
                # decoded_recieve = self.command_reader.read_recieve(recv)
    ...
    def set_zero_pos(self):
        ...
        # while decoded_recieve.id != command_answer_id:
        self.servo.send(channel=0, message=[set_zero_part_1,set_zero_part_2])
    ...
    def general_move_command(self):
        ...
        self.servo.send(channel=0, message=move_command)
    ...
    def save_settings(self):
        ...
        while decoded_recieve.id != command_answer_id:
            self.servo.send(channel=0, message=save_settings)
            recv = self.servo.receive(channel=0)
            decoded_recieve = self.command_reader.read_recieve(recv)
...
class ControlServoCan:
    def __init__(
        ...
        # проверка на наличие ключа в словаре
        if self.servo.servo_id not in servo_info_storage:
            servo_info_storage[self.servo.servo_id] = servo_realisation.commands_abstraction.commads_storage.ServoData(speed=-1, pos=-1, mode=-1)
        
        self.current_mode = self.read_mode()
        self.current_speed = self.read_speed()
        self.current_pos = self.read_pos()

        servo_info_storage[self.servo.servo_id] = servo_realisation.commands_abstraction.commads_storage.ServoData(speed=self.current_speed, pos=self.current_pos, mode=self.current_mode)
