...
class ControlServo:
    def __init__(self, servo_object: servo_realisation.servo_interface.ServoInterface) -> None:
        ...
        self.servo_commander = servo_realisation.commands_constructor.input_output_realisation.ServoCommander(servo_object=servo_object)

        check_mode = self.servo_commander.commands_constructor_abs.create_command(command_from_documentation='60600008', is_read=1, address=601)
        self.current_mode = -1


    def set_movements_speed(self, speed: int):
        set_speed = self.servo_commander.create_command(
            command_from_documentation="60810020", is_write=1, address=0x601, write_value=speed
        )
        time.sleep(0.2)
    ...
    def simple_move_to_position(self, position: int):
        ...
        move_to_position = self.servo_commander.create_command(
            command_from_documentation="607A0020", is_write=1, address=0x601, write_value=position
        )
    ...
    def simple_interpolation(self, position: int):
        send_to_all = self.servo_commander.create_command(
            write_value=128
        )
